...
(defmethod print-json-value ((value number) output-stream)
  (let* ((result (format nil "~,,,,,,'eE" value))
    ...
    (cond ((null decimal-position))
          ((= (1+ decimal-position) (length result))
           (setf result (subseq result 0 decimal-position)))
          ((not (digit-char-p (char result (1+ decimal-position))))
           (setf result (concatenate 'string
                                     (subseq result 0 decimal-position)
                                     (subseq result (1+ decimal-position))))))
    (let ((exponent-position (position #\e result))
          (end (length result)))
      (when (and (equal exponent-position (- end 3))
                 (char= #\0 (char result (1- end))))
        (setf result (subseq result 0 (- end 3))))
      (setf result (remove #\+ result))

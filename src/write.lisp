...
(defmethod print-json-value ((value float) output-stream)
  (multiple-value-bind (significand exponent sign)
      (quaviver:float-integer *quaviver-client* 10 value)
    (when (minusp sign)
      (write-char #\- output-stream))
    (prin1 significand output-stream)
    (unless (zerop exponent)
      (write-char #\e output-stream)
      (prin1 exponent output-stream))))

(defmethod print-json-value ((value number) output-stream)
  (print-json-value (coerce number 'double-float) output-stream))
